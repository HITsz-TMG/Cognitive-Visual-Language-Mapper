...
from llava.model.language_model.llava_llama import KnowledgeLlavaLlamaForCausalLM, KnowledgeOPTLlavaLlamaForCausalLM, KnowledgeOPTAttentionLlavaLlamaForCausalLM, KnowledgeSAMOPTAttentionLlavaLlamaForCausalLM, \
                            KnowledgeSAMOPTQueryAttentionLlavaLlamaForCausalLM, KnowledgeSigmoidOPTAttentionLlavaLlamaForCausalLM, KnowledgeOPTQformerLlavaLlamaForCausalLM
...
class ModelArguments:
    ...
    lora_load: Optional[bool] = field(default=False) # 加载lora参数但冻结lora继续训练
...
class DataArguments:
    ...
    sam_image_folder: Optional[str] = field(default="/data/cxy/Knowledge_LLaVA/playground/knowledge_qa/sam/images")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if os.path.exists(sam_image_folder):
                ...
                if len(sam_images) < 3:
                    sam_images = sam_images + [pil_image] * (3 - len(sam_images))
            else:
                print(f"sam gloabl: {image_file}")
                # sam_images = [pil_image, pil_image, pil_image, pil_image, pil_image]
                sam_images = [pil_image, pil_image, pil_image]
                # sam_images = [pil_image]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    opt_tokenizer = transformers.AutoTokenizer.from_pretrained("/data/share/Model/opt-1.3b", add_bos_token=True)
